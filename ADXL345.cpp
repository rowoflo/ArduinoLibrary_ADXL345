/*
 ...
 Initially created Bildr.org
 Modified by Rowland O'Flaherty (rowlandoflaherty.com) on 8/5/11.
...
#define ADXL345_TO_READ (6)      // num of bytes we are going to read each time
...
// Copy constructor
ADXL345::ADXL345(const ADXL345& srcObj) {
    copyHelper(srcObj);
}

// Copy helper function
void ADXL345::copyHelper(const ADXL345& srcObj) {
...
// Assignment operator
const ADXL345& ADXL345::operator=(const ADXL345& rhsObj) {
...
void ADXL345::rollPitch(double& theta, double& phi) {
    double xyz[3];
    get_Gxyz(xyz);
    
    double x = max(min(xyz[0],1),-1);
    double y = max(min(xyz[1],1),-1);
    double z = max(min(xyz[2],1),-1);
    
    if (z >= 0) {
        theta = atan2(y, sqrt(pow(z,2) + pow(x,2)));
        phi = atan2(-x, sqrt(pow(z,2) + pow(y,2)));
    } else {
        theta = atan2(y, -1 * sqrt(pow(z,2) + pow(x,2)));
        phi = atan2(-x, -1 * sqrt(pow(z,2) + pow(y,2)));        
    }
}
